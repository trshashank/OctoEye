...
#include <stdexcept>
#include <vector>
#include </home/samiarja/Desktop/PhD/Code/OctoEye/eigen-3.4.0/Eigen/Dense>
#include </home/samiarja/Desktop/PhD/Code/OctoEye/eigen-3.4.0/unsupported/Eigen/MatrixFunctions>
...
static PyObject* accumulate_pixel_map(PyObject* self, PyObject* args) {
    ...
    try {
        ...
        for (npy_intp index = 0; index < size; ++index) {
            ...
            event_indices[static_cast<npy_intp>(yi)][static_cast<npy_intp>(xi)].push_back(index);
            event_indices[static_cast<npy_intp>(yi)][static_cast<npy_intp>(xi) + 1].push_back(index);
            event_indices[static_cast<npy_intp>(yi) + 1][static_cast<npy_intp>(xi)].push_back(index);
            event_indices[static_cast<npy_intp>(yi) + 1][static_cast<npy_intp>(xi) + 1].push_back(index);
        }
        Py_END_ALLOW_THREADS;
        ...
        for (const auto& row : event_indices) {
            ...
            for (const auto& col : row) {
                ...
                for (const auto& idx : col) {
                    PyList_Append(colList, PyLong_FromLong(idx));
                }
...
static PyMethodDef octoeye_extension_methods[] = {
...
static struct PyModuleDef octoeye_extension_definition = {
    PyModuleDef_HEAD_INIT,
    "octoeye_extension",
    "octoeye_extension speeds up some sessiontools operations",
    -1,
    octoeye_extension_methods};
PyMODINIT_FUNC PyInit_octoeye_extension() {
    auto module = PyModule_Create(&octoeye_extension_definition);
    import_array();
    return module;
}
